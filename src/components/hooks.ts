...
export function initState(props: any, emit: any) {
  ...
  const [parentScale, setParentScale] = useState<number>(props.parentScale);
...
export function initDraggableContainer(
  ...
  const handleDrag = (e: MouseEvent) => {
    ...
    const deltaX = (pageX - lstPageX) / parentScale.value;
    const deltaY = (pageY - lstPageY) / parentScale.value;
